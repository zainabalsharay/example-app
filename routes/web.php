...
Route::get('/main3/{name}/{id}',function(string $name,int $id){
    return $name.' '.$id;})->where(['name' => '[a-z]+','id' => '[0-9]+']);

Route::get('/main4/{id}',function(int $id){
    return $id." this is number";})->whereNumber('id');

Route::get('/main5/{name}',function(string $name){
    return $name." this is alpha";})->whereAlpha('name');

Route::get('/main6/{name}',function(string $name){
    return $name."this is alpha and numeric";})->whereAlphaNumeric('name');

Route::get('/category/{category}', function (string $category) {
    return 'Category: ' . $category;})->whereIn('category', ['movie', 'song', 'painting']);

Route::get('/category/{category}', function (CategoryEnum $category) {
    return 'Category: ' . $category->value;})->whereIn('category',array_map(fn($case)=> $case->value,CategoryEnum::cases()));
